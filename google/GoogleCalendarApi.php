...
class GoogleCalendarApi { 
    ...
    public function DeleteCalendarEvent($event_id, $calendar_id, $access_token) {
      ...
      // path of the log file where errors need to be logged
       ...
       error_log("GOOGLE DELETE RESPONSE:".$http_code, 3, $log_file);
      if($http_code != 204 && $http_code != '' && $http_code != 2040 && $http_code != 410) { 
         return 0;
      }
      return 1;
   }
    ...
    public function CreateCalendarEvent($access_token, $calendar_id, $event_data, $all_day, $event_datetime, $event_timezone) { 
        ...
        $data = json_decode(curl_exec($ch), true); 
        $http_code = curl_getinfo($ch,CURLINFO_HTTP_CODE);   
        error_log("CreateCalendarEvent:".$http_code, 3, $log_file);      
        if ($http_code != 200) { 
